...
def parse_option():
    ...
    # Interediate layer
    parser.add_argument('--layer_inv', type=int, default=2, help='Intermediate Layer to constraint uniformity')
...
def main():
    ...
    for epoch in range(opt.epochs):
        ...
        for ii, (im_aug1, im_aug2) in enumerate(loader):
            ...
            out, intermediate = encoder(torch.cat([im_aug1.to(opt.gpus[0]), im_aug2.to(opt.gpus[0])]), intermediate_layer=opt.layer_inv )
            aug_1, aug_2 = intermediate.chunk(2)
            out_1, out_2 = out.chunk(2)

            align_loss_val = align_loss(aug_1, aug_2, alpha=opt.align_alpha)
            #clean = encoder(im_clean.to(opt.gpus[0]))
            unif_loss_val = koza_leon(out_1, k = opt.knn)+koza_leon(out_2, k = opt.knn)/2
